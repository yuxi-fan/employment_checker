...
def app():
    option = st.selectbox(
     ...
     ('COVID vs Unemployment Claims','Employment level change of different size company', 'Employment level change of different industry',"Unemployment Claims",'COVID vs Employment Level'))

    if option == 'Employment level change of different size company':
        st.title("Employment level change of different size company")
        HtmlFile = open("frontend/employment_company.html", 'r', encoding='utf-8')
        source_code = HtmlFile.read() 
        print(source_code)
        components.html(source_code,width = 1200,height = 900)

    if option == 'Employment level change of different industry':
        st.title("Employment level change of different industry")
